...
% irbc_N_new.m 
%
% This is an IRBC model with N countries. Prepared for EC611, 2018
% The model is solved as a social planner's problem (i.e., complete asset
% markets). 
%
% C. House 9/26/2018
% ================================================================================= % 
...
nimp_a 	= 15;		% number of years in impulse response
nplot_a = 4;    	% number of years to be plotted
h       = 1/12;     % fraction of a year per period (h=1 is one year)

N_plot  = [1 3 5 7 9 11];    % which countries to plot
...
beta    = exp(-h*r_ann);        % per period discount factor. 
...
rho_ann 	=  0;      % annual persistence

rho 	=  rho_ann^h;   % per period persistence
rho1 	=  .9;          % per period spillover from i to i+1
...
xnum = (10*N)+1; 	%  Number of variables in system 
...
for i = 1:N; 
    ...
    cof((8*N)+1,Yzero(i))   = -1;
    cof((8*N)+1,Izero(i))   = I/Y;
    cof((8*N)+1,Czero(i))   = C/Y;
...
for j = 1:length(N_plot); 
 ...
 colordef white
 figure(M+1) 
 plot(dat,impdat(1:nplot,Zpos(N_plot(1))),'b-'); 
 hold on; 
 for j = 2:M; 
     plot(dat,impdat(1:nplot,Zpos(N_plot(j))),'b-');
 end; 
 hold off; 
 title('Productivity')
 set(gca,'Xtick',yr_count);
 ...
 colordef white
 figure(M+2) 
 plot(dat,impdat(1:nplot,Ipos(N_plot(1))),'b-'); 
 hold on; 
 for j = 2:M; 
     plot(dat,impdat(1:nplot,Ipos(N_plot(j))),'b-');
 end; 
 hold off; 
 title('Investment')
 set(gca,'Xtick',yr_count);
