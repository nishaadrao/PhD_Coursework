...
\frametitle{Solving for steady state}
\begin{enumerate}
\item Guess an initial level of s.s dividends $\bar D^0=0$.
...
\item Compute the implied level of dividends, $\tilde{D}^0 = \bar C^0\times(1-\bar{W})$.
\item Update the guess of dividends: $\bar D^1 = \omega  \tilde{D}^0 + (1-\omega)\bar D^0$. I set $\omega=0.5$.
\item Iterate until convergence: $\bar D^n=\bar D^{n-1}$.
\end{enumerate}
